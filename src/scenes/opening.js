...
vec3 render(vec2 uv, vec2 fc){
  ...
  col *= 1. + smoothstep(uDur - .75, uDur, t)*6.;
  return col;
}
`;

// 1. Sanctum -- a temple hall at night; a star-shaped rift tears open.
const sanctumGLSL = /* glsl */ `
uniform sampler2D uFloorC, uFloorN, uLacqC, uLacqN, uWoodC, uWoodN;
uniform sampler3D uScreenS, uScreenCol, uLanternS, uLanternCol;
uniform float uScreenR, uLanternR;
uniform vec3 uScreenE, uLanternE;

const vec3 PORTAL = vec3(0., 2.4, 9.);
float portalR(float t){ return 2.3*smoothstep(1., 3.4, t)*(1.+.04*sin(t*13.)) + .35*smoothstep(uDur - 3.2, uDur - 1.8, t); }
...
// folding screens stand in every bay; paper lanterns hang between the pillars
vec3 screenLocal(vec3 p){ return vec3(mod(p.z, 4.) - 2., p.y - uScreenE.y*1.55, abs(p.x) - 7.45)/1.55; }
vec3 lanternLocal(vec3 p){ return vec3(abs(p.x) - 3.4, p.y - 5.3, mod(p.z, 4.) - 2.)/.8; }
vec3 lanternPos(int i){
  float side = (i%2==0) ? -1. : 1.;
  float z = float(i/2)*4. - 2.;
  return vec3(side*3.4, 5.3, z);
}
...
float map(vec3 p, out float m){
  ...
  vec3 q = p; q.x = abs(q.x) - 5.2; q.z = mod(q.z+2., 4.) - 2.;
  ...
  if(beam<d){ d=beam; m=1.; }
  float st = sdBox(p-vec3(0,.12,12.), vec3(4.,.12,2.2));
  st = min(st, sdBox(p-vec3(0,.36,13.), vec3(3.,.12,1.4)));
  if(st<d){ d=st; m=4.; }
  float fs = sdVol(uScreenS, uScreenR, uScreenE, screenLocal(p))*1.55;
  if(fs < d){ d = fs; m = 5.; }
  float ln = sdVol(uLanternS, uLanternR, uLanternE, lanternLocal(p))*.8;
  ln = min(ln, sdCylY(vec3(abs(p.x) - 3.4, p.y - 7., mod(p.z, 4.) - 2.), .015, 1.5));   // chains
  if(ln < d){ d = ln; m = 6.; }
  return d;
}
...
vec3 render(vec2 uv, vec2 fc){
  float t = uT;
  // camera: a slow, reverent dolly towards the rift, then yanked in
  float pull = smoothstep(uDur - 2.7, uDur - 1.3, t);
  vec3 ro = vec3(sin(t*.25)*.5 - .6, 1.7 + .1*sin(t*.45), -5.5 + t*.5 + pull*pull*12.);
  ...
  vec3 pc = vec3(.45,.7,1.)*(R*R)*1.6*flick;
  ...
  if(d < 60.){
    vec3 n = nrm(p), v = -rd;
    float o = ao(p, n);
    Mat mt; mt.alb = vec3(.25,.12,.07); mt.rough = .5; mt.n = n; mt.ao = 1.; mt.h = .5;
    vec3 emit = vec3(0);
    if(m == 0. || m == 4.){                        // polished granite floor
      mt = planarMat(uFloorC, uFloorN, p.xz*.22, n, .6);
      mt.alb *= .55; mt.rough *= .45;
      float ring = abs(length(p.xz-vec2(0,6.))-3.5);
      mt.alb = mix(mt.alb, vec3(.5,.34,.12), smoothstep(.08,.0,ring)*.85);
    } else if(m == 1.){ mt = triMat(uWoodC, uWoodN, p*.5, n, 1., 1.); mt.alb *= .5; }
    else if(m == 2.){ mt = triMat(uLacqC, uLacqN, p*.6, n, 1., .6); mt.alb *= vec3(1.1,.55,.45); }
    else if(m == 3.){
      mt = triMat(uWoodC, uWoodN, p*.35, n, 1., 1.); mt.alb *= .6;
      float lat = lattice(vec2(p.z*.25, p.y*.25));
      float pane = smoothstep(.05,.09,lat)*step(abs(p.x),8.3)*step(7.9,abs(p.x))*step(1.2,p.y)*step(p.y,7.5);
      emit += vec3(1.,.5,.2)*pane*.3*(.6+.4*fbm(p.yz*.7));
      float win = length(p.xy-vec2(0,4.2));
      emit += vec3(.35,.45,.8)*.5*step(20.5,p.z)*smoothstep(3.2,2.8,win)*(.5+.5*smoothstep(.06,.1,lattice(p.xy*.35)));
    } else if(m == 5.){
      mt.alb = volAlbedo(uScreenCol, screenLocal(p)); mt.rough = .45;
      emit += mt.alb*vec3(1.,.55,.25)*.12;                  // lamplight glowing through the silk panels
    } else {
      vec3 c = volAlbedo(uLanternCol, lanternLocal(p));
      mt.alb = c; mt.rough = .6;
      float fl = .85 + .15*noise(vec2(t*8., floor(p.z/4.) + sign(p.x)));
      emit += c*vec3(1.,.55,.22)*5.*fl;                      // the paper glows from the candle inside
    }
    // light from the rift
    vec3 lv = PORTAL - p; float ld = length(lv); lv /= ld;
    col += litPBR(mt.alb, mt.rough, 0., mt.n, v, lv, pc/(ld*ld*.6+1.))*o;
    col += mt.alb*pc*.2*sat(dot(mt.n,lv)*.5+.5)/(ld*ld*.6+1.)*o;
    // lanterns
    for(int i=0;i<10;i++){
      vec3 lp = lanternPos(i); vec3 l = lp-p; float ll = length(l); l/=ll;
      float lf = .85+.15*noise(vec2(t*8.,float(i)));
      col += litPBR(mt.alb, mt.rough, 0., mt.n, v, l, vec3(1.,.45,.15)*1.7*lf/(ll*ll+.3))*o;
    }
    col += mt.alb*vec3(.15,.2,.35)*.25*(mt.n.y*.5+.5)*o;
    col += emit;
    if(m == 0. || m == 4.){
      vec3 rr = reflect(rd, mt.n);
      float fr = fresnel(mt.n, rd, .05);
      float tp = (PORTAL.z - p.z)/rr.z;
      if(tp > 0.){
        vec2 q = (p + rr*tp).xy - PORTAL.xy;
        vec4 sp = starPortal(q*vec2(-1,1), R, t);
        col += sp.rgb*fr*(1.-mt.rough)*1.6*exp(-tp*.05);
      }
      // lanterns mirrored in the stone
      for(int i=0;i<10;i++){
        vec3 lp = lanternPos(i); vec3 q = p + rr*max(dot(lp - p, rr), 0.) - lp;
        col += vec3(1.,.5,.2)*.012/(dot(q,q)+.01)*fr*(1.-mt.rough);
      }
    }
  }
  float tmax = min(d, 60.);
  for(int i=0;i<10;i++){
    ...
    col += vec3(1.,.42,.12)*fogLight(ro, rd, lp, tmax)*.012*(.85+.15*noise(vec2(t*8.,float(i))));
  }
  ...
  for(int i=0;i<6;i++){
    float fi = float(i);
    float on = step(.45, hash11(floor(t*7.)+fi*5.))*smoothstep(1.6,2.6,t);
  ...
  for(int i=0;i<18;i++){
    ...
    vec3 sp = PORTAL + (h-.5)*vec3(10.,6.,2.) - vec3(0,0,mod((t-1.5)*(1.+h.z*3.), 14.));
    vec3 q = ro + rd*max(dot(sp-ro, rd),0.) - sp;
    float s = length(q);
    col += vec3(.6,.85,1.)*.0025/(s*s+.0008)*smoothstep(1.5,2.5,t)*step(dot(sp-ro,rd),tmax);
  }
  col = mix(col, vec3(.7,.85,1.)*1.6, smoothstep(uDur - 1.4, uDur - .6, t)*.35);
  return col;
}
`;
...
export const sanctum = {
  glsl: sanctumGLSL,
  uses: {
    uFloor: 'mat:granite', uLacq: 'mat:lacquer', uWood: 'mat:darkwood',
    uScreen: 'vol:screen', uLantern: 'vol:lantern',
  },
};
