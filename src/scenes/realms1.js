...
const firehallGLSL = /* glsl */ `
uniform sampler2D uRockC, uRockN, uStoneC, uStoneN;
uniform sampler3D uBustS, uBustCol;
uniform float uBustR;
uniform vec3 uBustE;
const float BS = 4.2;          // colossus scale
// colossal marble busts line both walls, facing the aisle, half sunk in fire
vec3 bustLocal(vec3 p){
  vec3 q = vec3(abs(p.x) - 4.9, p.y - (-2.2 + uBustE.y*BS), mod(p.z, 6.) - 3.);
  return vec3(q.z, q.y, -q.x)/BS;
}
float map(vec3 p, out float m){
  float st = sdVol(uBustS, uBustR, uBustE, bustLocal(p))*BS;
  float wall = 6.8 - abs(p.x) + .5*noise(p*.6);
  float floor_ = p.y + 1.5 + .4*noise(p.xz*.5);
  float roof = 11. - p.y + .8*noise(p.xz*.3);
  float env = min(wall, min(floor_, roof)) + .06*noise(p*3.);
  m = st < env ? 1. : 0.;
  return min(st, env);
}
...
vec3 eyeGlow(vec3 ro, vec3 rd, float tmax, float t){
  ...
  for(int k=0;k<6;k++){
    ...
    for(int s=0;s<2;s++){
      float side = s==0 ? -1. : 1.;
      for(int e=0;e<2;e++){
        vec3 lp = vec3(side*(4.9 - .40*BS), -2.2 + (uBustE.y + .62)*BS, z + (e == 0 ? -.12 : .03)*BS);
...
vec3 render(vec2 uv, vec2 fc){
  ...
  if(d < 45.){
    ...
    Mat mt;
    if(m > .5){
      mt = triMat(uStoneC, uStoneN, p*.4, n, 1., 1.2);
      mt.alb = mix(volAlbedo(uBustCol, bustLocal(p)), mt.alb, .55)*vec3(.9,.8,.7);   // soot-stained marble
    } else {
      mt = triMat(uRockC, uRockN, p*.3, n, 1., 1.3);
      mt.alb *= .7;
    }
    vec3 alb = mt.alb; n = mt.n;
    ...
    for(int k=0;k<6;k++){
      ...
      for(int s=0;s<2;s++){
        ...
        col += litPBR(alb, mt.rough, 0., n, -rd, l/ll, vec3(1.,.45,.13)*16.*fl/(ll*ll+2.)) + alb*vec3(1.,.45,.13)*.2*16.*fl/(ll*ll+2.);
      }
    }
    col += ambPBR(alb, mt.rough, 0., n, -rd, mt.ao)*.35;
    // eye rim light
    vec3 zc = vec3(sign(p.x)*3.3, -2.2 + (uBustE.y + .62)*BS, floor(p.z/6.)*6.+3.);
...
export const firehall = {
  glsl: firehallGLSL,
  uses: { uEnv: 'env:furnace', uRock: 'mat:cliff', uStone: 'mat:rock', uBust: 'vol:bust' },
  env: { gain: 1.0 },
};

// 3. Deep space -- a crackling starburst over a planet's night side.
const cosmosGLSL = /* glsl */ `
uniform sampler3D uRockS, uRockCol;
uniform float uRockR;
uniform vec3 uRockE;
vec3 stars(vec3 rd){
  vec3 c = vec3(0);
  for(int i=0;i<3;i++){
...
vec3 render(vec2 uv, vec2 fc){
  ...
  // tumbling moon-rock debris flying past (baked from a scanned rock)
  for(int i=0;i<9;i++){
    float fi = float(i);
    vec3 hh = hash31(fi*5.3);
    vec3 sp = vec3((hh.x-.5)*6., (hh.y-.5)*3., mod(hh.z*20. - t*6., 20.) + ro.z);
    float sr = .2 + .3*hh.x;
    vec3 o2 = ro - sp; float b2 = dot(o2, rd), c2 = dot(o2,o2) - sr*sr*1.5, h2 = b2*b2 - c2;
    if(h2 > 0.){
      mat3 R = mat3(1.);
      float a1 = t*(.6 + hh.y) + fi, a2 = t*(.4 + hh.z) + fi*2.;
      float s = sr;
      float tt = max(-b2 - sqrt(h2), 0.);
      for(int k=0;k<28;k++){
        vec3 q = (ro + rd*tt - sp)/s;
        q.xy *= rot(a1); q.yz *= rot(a2);
        float dd = sdVol(uRockS, uRockR, uRockE, q)*s;
        if(dd < .002){
          vec2 e = vec2(.02, 0.);
          vec3 n = normalize(vec3(sdVol(uRockS,uRockR,uRockE,q+e.xyy)-sdVol(uRockS,uRockR,uRockE,q-e.xyy),
                                  sdVol(uRockS,uRockR,uRockE,q+e.yxy)-sdVol(uRockS,uRockR,uRockE,q-e.yxy),
                                  sdVol(uRockS,uRockR,uRockE,q+e.yyx)-sdVol(uRockS,uRockR,uRockE,q-e.yyx)));
          n.yz *= rot(-a2); n.xy *= rot(-a1);
          vec3 alb = volAlbedo(uRockCol, q)*1.2;
          float dif = sat(dot(n, normalize(vec3(-.5,.2,1.))));
          float rim = pow(1. - sat(dot(n, -rd)), 3.);
          col = alb*(.04 + dif*vec3(.5,.7,1.)*1.2) + vec3(.3,.55,1.)*rim*.8;
          break;
        }
        tt += dd;
        if(tt > -b2 + sqrt(h2) + .1) break;
      }
    }
  }
  return col;
...
export const cosmos = { glsl: cosmosGLSL, uses: { uRock: 'vol:moonrock' } };
...
vec3 env(vec3 rd, vec3 L){
  ...
  vec3 c = mix(vec3(.0,.04,.08), vec3(.1,.55,.8), pow(s, 3.)) + envLod(rd, 1.)*vec3(.25,.75,.95)*.5;
...
export const crystal = { glsl: crystalGLSL, uses: { uEnv: 'env:snow' }, env: { gain: 1.0 } };

// 6. The fall -- down a banded sandstone canyon towards a blinding light.
const canyonGLSL = /* glsl */ `
uniform sampler2D uStoneC, uStoneN, uMossC, uMossN;
...
vec3 render(vec2 uv, vec2 fc){
  ...
  if(d < 120.){
    ...
    Mat sm = triMat(uStoneC, uStoneN, p*.22, n, 1., 1.4);
    rock *= sm.alb*2.4;
    float moss = smoothstep(.55,.85,n.y + .3*fbm3(p*1.5))*step(-60., p.y);
    Mat mm = triMat(uMossC, uMossN, p*.4, n, 1., 1.);
    vec3 alb = mix(rock, mm.alb*vec3(.8,1.,.6), moss*.85);
    n = normalize(mix(sm.n, mm.n, moss));
    ...
    col += ambPBR(alb, .8, 0., n, -rd, 1.)*.35;
    col = mix(col, envLod(rd, 4.)*vec3(1.,.8,.6), 1.-exp(-d*.01));
  }
  // light at the end of the canyon
  float burst = smoothstep(uDur - .9, uDur - .2, t);
...
export const canyon = {
  glsl: canyonGLSL,
  uses: { uEnv: 'env:clear', uStone: 'mat:sandstone', uMoss: 'mat:mossrock' },
  env: { rot: 2.04, gain: 1.0 },
};
